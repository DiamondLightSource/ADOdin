...
class _ArcProcessPlugin(_FrameProcessorPlugin):
    ...
    def create_extra_config_entries(self, rank):
        entries = []
        dimensions_entry = {
            self.NAME: {"width": ARC_DIMENSIONS[0], "height": ARC_DIMENSIONS[1]}
        }
...
class _ArcOdinData(_OdinData):
    ...
    CONFIG_TEMPLATES = {"FrameProcessor": "fp_arc.json", "FrameReceiver": "fr_arc.json"}

    def __init__(self, server, READY, RELEASE, META, PLUGINS, BASE_UDP_PORT):
    ...
    def create_config_files(self, index):
        macros = dict(
            ...
            WIDTH=ARC_DIMENSIONS[0],
            HEIGHT=ARC_DIMENSIONS[1],
        )
...
--logserver="graylog2.diamond.ac.uk:12210" --access_logging=ERROR'.format(
    ...
    def fem_address_list(self):
        if self.FEMS_REVERSED:
            return ", ".join(reversed(self.CONFIG_TEMPLATES["fem_addresses"]))
        else:
            return ", ".join(self.CONFIG_TEMPLATES["fem_addresses"])
...
class ArcDetector(_OdinDetector):
    ...
    # We don't really need the OdinDataDriver, but we need to know it is instantiated
    ...
    def create_udp_file(self):
        fem_dests = []
        for offset in range(self.SENSOR_OPTIONS[1]):  # 2 for 1M or 6 for 3M
            fem_dests.append(
                #    "fems": [
                "        {{\n"
                '            "name": "fem{number}",\n'
    ...
    def generate_direct_fem_node_config(self):
        ...
        # Each will have its own list of entries, one for every receiver, with
        # different ports
        node_config = []
        id = 1
        for mac, ip in [
...
class _ArcGapFillPlugin(_FrameProcessorPlugin):
    ...
    def create_extra_config_entries(self, rank):
        ...
        dimensions = [ARC_DIMENSIONS[1] + sum(y_gaps), ARC_DIMENSIONS[0] + sum(x_gaps)]
        dataset_config = {
